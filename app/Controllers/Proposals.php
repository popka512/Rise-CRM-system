...
class Proposals extends Security_Controller {
    ...
    function modal_form() {
        ...
        if (!$this->is_proposal_editable($id, $is_clone)) {
    ...
    private function _make_row($data, $custom_fields) {
        ...
        $edit = "";
        if ($this->is_proposal_editable($data->id)) {
    ...
    function view($proposal_id = 0) {
        ...
        if ($proposal_id) {
            ...
            if ($view_data) {
                ...
                $view_data["is_proposal_editable"] = $this->is_proposal_editable($proposal_id);
    ...
    function discount_modal_form() {
        ...
        if (!$this->is_proposal_editable($proposal_id)) {
            app_redirect("forbidden");
        }
    ...
    function delete_item() {
        ...
        if (!$this->is_proposal_editable($item_info->proposal_id)) {
            app_redirect("forbidden");
        }
    ...
    private function _make_item_row($data) {
        ...
        if ($this->is_proposal_editable($data->proposal_id)) {
    ...
    private function is_proposal_editable($proposal_id = 0, $is_clone = 0) {
        if (!$proposal_id || $is_clone) {
            return true;
        }

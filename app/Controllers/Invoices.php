...
class Invoices extends Security_Controller {
    ...
    private function _make_row($data, $custom_fields) {
        ...
        if ($this->login_user->user_type == "staff") {
            ...
            $invoice_url = anchor(get_uri("invoices/view/" . $data->id), get_invoice_id($data->id, $data->due_date), array("class" => $link_class)) . $main_invoice_url . $credit_note_url;
        } else {
            $link_class = "";
            if ($data->main_invoice_id) {
    ...
    private function _make_options_dropdown($data) {
        $edit = '';
        if ($this->is_invoice_editable($data->id)) {
            $edit = '<li role="presentation">' . modal_anchor(get_uri("invoices/modal_form"), "<i data-feather='edit' class='icon-16'></i> " . app_lang('edit'), array("title" => app_lang('edit_invoice'), "data-post-id" => $data->id, "class" => "dropdown-item")) . '</li>';
        }
